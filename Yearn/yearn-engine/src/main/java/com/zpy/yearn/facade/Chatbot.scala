...
import com.zpy.yearn.service.{ChSentenceService, Inferers, 机器人的头脑}
...
class Chatbot {
  ...
  def heard(fromWho: String, sentencesStr: String): PsychoCounselContext = 收到句子(fromWho, sentencesStr)

  def 接收常识(sentencesStr: String) = {
    ...
    for (stcStr <- sentenceStrList) {
      if (机器人的头脑.all().map(_.rawStcStr == stcStr).nonEmpty && 机器人的头脑.all().exists(_.rawStcStr == stcStr)) {
        //stc exists, do nothing
      } else {
        val parseStcs: util.List[TPred] = chSentenceService.stc2Clause(stcStr, chatbotContext)
        import collection.JavaConverters._
        for (stc <- parseStcs) {
          if (!机器人的头脑.tryPut2clauseTemp(parseStcs.asScala)) {
            机器人的头脑.receive(stc, sentencesStr, 10)
    ...
    for (stcStr <- sentenceStrList) {
      ...
      for (stc <- parseStcs) {
        if (stcStr.contains("?") || stcStr.contains("？")) {
          chatbotContext.回答 += YBot.answer(stc)
          chatbotContext.heardStcs += stc
        } else {
          import collection.JavaConverters._
          if (!机器人的头脑.tryPut2clauseTemp(parseStcs.asScala)) {
            机器人的头脑.receive(stc, sentencesStr, 6)
            chatbotContext.heardStcs += stc
          }
        }
      }
  ...
  /**
    ...
    * @param sentencesStr
    */
  def 收到句子(fromWho: String, sentencesStr: String): PsychoCounselContext = {
    ...
    for (stcStr <- sentenceStrList) {
      ...
      for (stc <- parseStcs) {
        ...
        if (!机器人的头脑.tryPut2clauseTemp(parseStcs.asScala)) {
          ...
          机器人的头脑.receive(heardSay, sentencesStr, 9)
  ...
  def stcStr2SenseObj(stc: String): util.List[TPred] = chSentenceService.stc2Clause(stc, new BotReadContext)
