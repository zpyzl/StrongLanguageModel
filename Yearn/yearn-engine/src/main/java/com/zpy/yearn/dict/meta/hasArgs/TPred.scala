...
trait TPred extends TThing with HasArgs with HasElements{
  ...
  def causes: Set[TPred] = {
    val knowledgeReasons = 机器人的头脑.all().flatMap {
      case cause: Cause =>
...
/*
  ...
  def will(): this.type = {
    copyReplaceMods(mods.filter(!_.isInstanceOf[TenseMark]) + Will())
    tense_=(Tense.SimpleFuture)
  }

  def did(): this.type = {
    copyReplaceMods(mods.filter(!_.isInstanceOf[TenseMark]) + Did())
    tense_=(Tense.SimplePast)
  }
