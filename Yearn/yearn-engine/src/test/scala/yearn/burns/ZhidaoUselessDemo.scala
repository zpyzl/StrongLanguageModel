  ...
  def doNotGood(girl: Person ): Boolean = {
    val expected = Adv.not(
      Be( Result().of( DoVT(girl,  AnyEntity())),
        Good()))
     机器人的头脑.all().contains(expected
       )
  }
  def zhidaoUselessExample( chatbot: Chatbot) : Boolean = {
    val botContext = chatbot.收到句子("22YearOldGirlFromZhidao", "我觉得自己一无是处，什么都做不好。只弹了两首不完整的歌") //也做不会。做事坚持不了没毅力。越来越自卑，越来越没自信。喜欢钢琴，学了两年就没有再学，至今为止只弹的了两首不算完整的歌");
    val girl = new Person().nameIs( "22YearOldGirlFromZhidao")
