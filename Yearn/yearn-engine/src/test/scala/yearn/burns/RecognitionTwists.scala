...
object RecognitionTwists extends FlatSpec {
  def zsIsExtremeEvalSelf(chatbot: Chatbot): Boolean = {
    ...
    val zsSayUseless = chatbot.收到句子("张三", "我一无是处").heardSayings.head
  ...
  def greet(chatbot: Chatbot): Boolean = {
    ...
    val zxy = new Person().nameIs( "张学友")
    val ldh = new Person().nameIs( "刘德华")
     ...
     */
    val botContext = chatbot.收到句子("张学友", "刘德华认识我、黎明和郭富城。他没有向我打招呼。")
  ...
  //Person zhangsan = new Person().nameIs(" 张三");
