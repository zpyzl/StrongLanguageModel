...
答：每个人的性格中，都有某些无法让人接受的部分
  ...
  def persuade(chatbot: Chatbot): Boolean = {
    chatbot.接收常识("想让别人相信自己的看法，就要冷静地表达看法。")

    val context = chatbot.收到提问("Jack","怎么说服别人？")
    context.answerStrs.contains("就要冷静地表达看法")
  }

  def 用已知回答(已知: String, 机器人的反应: SearchContext ): Boolean = {
    if( 机器人的反应.回答.isEmpty || 机器人的反应.回答.head.isEmpty) throw new RuntimeException("no answer!")
    机器人的反应.回答.head.head.asInstanceOf[TPred].stcStr == 已知
  }
  /*
重要：主要：
你最重要任务是A。
