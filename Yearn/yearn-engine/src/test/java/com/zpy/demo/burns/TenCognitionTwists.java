...
use应该属于advb，应该在谓语动词解释后去解释。怎么想办法去修饰谓语动词的fm
    ...
    public void zsIsExtremeEvalSelf() throws YearnException {
        chatbot.inputCommonSense(
                "你用一种极端的标准评价自己。" );
        期望(RecognitionTwists.zsIsExtremeEvalSelf(chatbot));
    }*/
    ...
    public void greet() {
         ...
         */
        期望( RecognitionTwists.greet(机器人)) ;
    }
...
那些话语听得太多，越来越自卑，越来越没自信的我，除了听歌和上网能够让自己安心，其他什么都不能。。
    ...
    public void zhidaoUselessExample(){
        期望(ZhidaoUselessDemo.zhidaoUselessExample(机器人));
    }
    /*
...
影响：改变
  */
    @Test
    public void 只有你的思想才能影响你的情绪(){

        机器人.接收常识("只有你的思想才能影响你的情绪。有些人容易相信别人。");
        PsychoCounselContext context =
                机器人.收到句子("刘德华", "张学友说我没用。所以我变得很沮丧");
        /*
        **只有** 思想 导致 情绪
        张学友说我没用 导致 沮丧  ——错误
         */
        期望(TestEvaluate.evalShouldNotCauseMoodDirectly(context));
        /*
        测试“沮丧”的理解：
        刘德华的情绪怎么样？
        刘德华高兴吗？
...
//t1
    private void 期望(boolean res){
        Assert.assertTrue(res);
    }
    private void 期望否定(boolean res){
        Assert.assertTrue(!res);
    }
    @Test
    public void 情绪怎么样(){
        String 已知 = "张学友说我没用。所以我变得很沮丧";
        PsychoCounselContext knownContext =
                机器人.收到句子("刘德华", 已知);
        SearchContext 机器人的反应 =
                机器人.收到提问("user1", "刘德华的情绪怎么样？");
        期望(TestSenses.用已知回答( 已知, 机器人的反应));
    }

    @Test
    public void 困难(){
    机器人.接收常识("克服困难都是痛苦的");
    PsychoCounselContext knownContext = 机器人.收到句子("刘德华", "张学友经常克服困难");
    SearchContext 机器人回答 = 机器人.收到提问("user1", "张学友勇敢吗?");
        期望(1==2);
        //因为 "张学友经常克服困难");
    }
...
have desire of  , not conform props of reality
         */
        PsychoCounselContext knownContext =
                机器人.收到句子("刘德华", "张学友想学开车");
        SearchContext 机器人回答 =
                机器人.收到提问("user1", "张学友的目标是什么?");
        期望(TestSenses.用已知回答( "张学友想学开车",机器人回答));
    }

   /* @Test
    public void 目标和想要_反例(){
        PsychoCounselContext knownContext =
                机器人.收到句子("刘德华", "张学友在开车想事情");
        SearchContext 机器人回答 =
                机器人.收到提问("user1", "张学友的目标是什么?");
        期望否定(TestSenses.用已知回答( "张学友在开车想事情",机器人回答));
    }*/
    /*
...
mood be a bad mood because not satisfy =>
    ...
    public void 为什么不满意(){
        String 已知 = "张学友说我没用。所以我变得很沮丧";
        PsychoCounselContext knownContext = chatbot.heardInPsychoCounsel("刘德华", 已知);
        SearchContext 机器人回答 = chatbot.heardInSearch("user1", "刘德华为什么不满意？");
        期望(TestSenses.以语料回答(机器人回答, 已知));
        //根据打招呼的定义，回答打招呼是因为什么？
    }*/
    /*
...
怎么说服别人？=》说服别人的方式是？=> AM(sb, ?, letBelieve, _ )
   ...
   /* @Test
    public void important(){
        期望(TestSenses.important(chatbot));
    }
*/
   /*
   帮助：
...
/*
    @Test
    public void greP17(){
        期望(GREPassagesDemo.p17q2(chatbot));
    }*/
/*
